...
//		System.out.println("totalPage : " + totalPage );
	...
	public ItemVO readItemById(String iRegister) {
		return dao.selectItemById(iRegister);
	}
...
<<<<<<< HEAD
	...
	public ItemOptionVO readOption(int iNum) {
		...
		return dao.selectItemOption(iNum);
	}
	
	public boolean insertItemOption(ItemOptionVO io, String loginId) {
		ItemVO item = dao.selectItem(io.getiNum());
		int insert=0;
		if (item.getiRegister().equals(loginId)) {
			insert = dao.insertItemOption(io);			
		}
		boolean result = insert==1 ? true:false;
		return result;
	}
	
	public boolean updateItemOption(ItemOptionVO io, String loginId) {
		ItemVO item = dao.selectItem(io.getiNum());
		int update=0;
		if(item.getiRegister().equals(loginId)) {
			update = dao.updateItemOption(io);
		}
		boolean result = update==1 ? true:false;
		return result;		
	}
}
=======
